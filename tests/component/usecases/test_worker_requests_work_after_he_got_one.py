...
class TestCreateAndFinishWork:
    async def test_create_and_finish_work(
        ...
        worker_client_calls = worker_client.send.call_count
        await worker_requests_work_usecase.perform(fixt_worker.worker_id)
        assert worker_client_calls == worker_client.send.call_count

        worker_2 = Worker(
            worker_id='1.2.3.4:91911',
            worker_ref=sentinel.worker_2_ref
        )
        await worker_connected_usecase.perform(worker_2)
        await worker_requests_work_usecase.perform(worker_2.worker_id)
